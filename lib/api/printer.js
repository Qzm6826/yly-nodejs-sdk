...
class Printer{
     ...
     * @returns {*}
     */
    pushSwitch(machineCode, status) {
        if(_.isEmpty(String(machineCode))) {
            throw new Error('machine_code can not be empty');
        }
        if(_.isEmpty(String(status))) {
            throw new Error('status can not be empty');
        }
        return this.rpcClient.call('printer/pushswitch', {machine_code : machineCode, status : status});
    }
    ...
    setKeyWords(machineCode, keys, type, content) {
        ...
        return this.rpcClient.call('printer/setkeywords', {machine_code : machineCode, keys : keys, type : type, content : content})
    }
     ...
     * @param usbPrintMode
     * @param usbInputMode
     * @param cameraDecodeTxMode
     * @returns {*}
     */
    setting(machineCode, usbPrintMode = null, usbInputMode = null, cameraDecodeTxMode = null) {
        var data = {};
        if (_.isEmpty(String(machineCode))) {
            throw new Error('machine_code can not be empty');
        }
        data['machine_code'] = machineCode;
        if (!_.isEmpty(String(usbPrintMode))) {
            data['usb_print_mode'] = parseInt(usbPrintMode);
        }
        if (!_.isEmpty(String(usbInputMode))) {
            data['usb_input_mode'] = parseInt(usbInputMode);
        }
        if (!_.isEmpty(String(cameraDecodeTxMode))) {
            data['camera_decode_tx_mode'] = parseInt(cameraDecodeTxMode);
        }
        return this.rpcClient.call('printer/setting', data);
    }
